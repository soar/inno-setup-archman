...
type
  ...
  function CreateInArchive(const classid: TGUID; const libDir: string): I7zInArchive;
  function CreateOutArchive(const classid: TGUID; const libDir: string): I7zOutArchive;
...
function CreateInArchive(const classid: TGUID; const libDir: string): I7zInArchive;
begin
  //SetDllDirectory(PChar(libDir));
  Result := T7zInArchive.Create(libDir + '\7z.dll');
...
begin
  //SetDllDirectory(PChar(libDir));
  Result := T7zOutArchive.Create(libDir + '\7z.dll');
  Result.ClassId := classid;
end;
...
begin
  ...
  if FHandle = 0 then
    raise exception.CreateFmt('#563 - Error loading library %s', [lib]);
